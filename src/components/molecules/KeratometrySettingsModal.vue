        ...
        <div v-if="show" class="modal-overlay" @click.self="$emit('close')">
            <div class="settings-modal">
                <div class="modal-header">
                    <h2>{{ t('keratometry.title') }} - {{ t('settings.title') }}</h2>
                ...
                <div class="modal-body">
                    ...
                    <template v-else>
                        ...
                        <div class="fields-list" :key="configKey">
                            <!-- Keratometry Fields Only -->
                            <div v-if="config && config.machine" class="section-group">
                                <h3 class="section-title">{{ t('keratometry.title') }}</h3>
                                <div 
                                    v-for="(fieldConfig, fieldKey) in keratometryFields" 
                                    :key="`keratometry-${configKey}-${fieldKey}`"
                                    class="field-item"
                                >
                                    <div class="field-row">
...
// Only show keratometry-related fields
const keratometryFieldKeys = ['K1', 'K2', 'avgKm', 'axK1'];

const keratometryFields = computed(() => {
    if (!config.value || !config.value.machine) return {};
    const filtered = {};
    keratometryFieldKeys.forEach(key => {
        if (config.value.machine[key]) {
            filtered[key] = config.value.machine[key];
        }
    });
    return filtered;
});
...
async function resetAllToDefaults() {
    // Reset only keratometry fields
    try {
        for (const field of keratometryFieldKeys) {
            await window.api.config.resetFieldToDefault('machine', field);
        }
        await loadConfig(false); // Don't show loading during reset
        await nextTick();
        await loadFieldRules();
...
async function saveChanges() {
    saving.value = true;
    try {
        // Save only keratometry fields
        const plainConfig = JSON.parse(JSON.stringify(config.value));
        
        for (const field of keratometryFieldKeys) {
            if (plainConfig.machine && plainConfig.machine[field]) {
                const fieldConfig = plainConfig.machine[field];
